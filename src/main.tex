...
passed it ten minutes ago! Faster!'}{Lewis Carroll},
  indexkeys={1.~Computação --- Teses. 2.~Compiladores --- Teses. I.~Orientador.
    II.~Título.},
}
...
%- Context: pointers are important
%- Problem: analyzing them is difficult
%- Solution: a new points-to analysis
...
offsets, as explained by Yong and Horwitz~\cite{Yong04}.
...
We let $\gr(p)$ be the global abstract address set set associated with pointer $p$, such that if
...
\textbf{The key insight of this paper} is the combination of pointer analysis
with range analysis on the symbolic interval lattice.
In a symbolic range analysis, ranges are defined as expressions of the program
...
The algorithms that we present in this paper do not depend on any particular
implementation.
...
As we show in Section~\ref{sec:exp}, our analysis is linear on the size of
programs.
...
Steensgard's~\cite{Steensgaard96}, Andersen's~\cite{Andersen94}, or even
the state-of-the-art technique of Hardekopf and Lin~\cite{Hardekopf11}.
...
acceleration techniques such as lazy cycle detection~\cite{Hardekopf07a}, or
wave propagation~\cite{Pereira09b}.
...
Michael Wolfe~\cite[Ch.7]{Wolfe96} and Aho {et al.}~\cite[Ch.11]{Aho06} have entire
...
Even Rugina and Rinard~\cite{Rugina05}, who we believe is the state-of-the-art
approach in the field today, use integer linear programming to solve symbolic
relations between variables.
We speculate that the ILP approach is too expensive to be used in large programs;
hence, concessions must be made for the sake of speed.
...
Cousot~\cite{Cousot77}, whereas we use symbolic intervals.
The inspiration for much of this work springs from Balakrishnan and Reps notion
of {\em Value Set} Analysis~\cite{Balakrishnan04}.
Integer intervals have also being used by Yong {\em et al.}~\cite{Yong04} and,
more recently, by Oh {\em et al.}~\cite{Oh14}.
...
Finally, the fact that we use Bodik's e-SSA form~\cite{Bodik00} distinguish our
...
%Chapter 4: A New Points-to Analysis
...
In this paper, we propose an alias analysis that solves this problem.
...
Blume~\cite{Blume94}, to bootstrap our pointer analysis.
...
form~\cite{Bodik00}.
E-SSA form is a flavor of Static Single Assignment (SSA)~\cite{Cytron91} form,
...
The implementation of the analysis that we shall present in this paper is
...
points.
As demonstrated by Choi {\em et al.}~\cite{Choi91}, the main advantage of a sparse
...
range analysis, which we borrow from Nazar\'{e} {\em et al.}~\cite{Nazare14}.
...
In this paper, we are using the following widening operator on \ranboxes:
 \[[l, u]\nabla [l', u'] \ = \
\begin{cases}
...
A range analysis such as Nazar\'{e} {\em et al.}'s~\cite{Nazare14}, if
...
arises.
According to Tavares {\em et al.}~\cite{Tavares14}, this property -- single
information -- is sufficient to enable sparse implementation of non-relational
static analyses~\cite{Tavares14}.
...
form~\cite{Bodik00}. Figure~\ref{fig:exWidening} shows the result
...
capabilities of the technique that we present in this paper, namely the
...
In this paper we have presented a new alias analysis technique that handles,
...
In future work, we plan to investiguate better splitting strategies
